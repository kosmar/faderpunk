...
/// index, strum ms). First entry differs from factory defaults so the first
/// step is always audible (harmony drops an octave).
...
/// One hue per preset. Even 30° steps crowd cyan/blue on WS2812s; golden-angle
/// spacing (~137°) keeps the twelve stops visually distinct (only one true blue).
fn preset_color(idx: usize) -> Color {
    let h = ((idx % NUM_PRESETS) * 137 % 360) as u16;
    let (r, g, b) = hsv_to_rgb(h, 255, 255);
    Color::Custom(r, g, b)
}

/// Compact HSV→RGB for LED feedback (h in degrees 0..359).
    ...
    let engine = async {
        ...
        loop {
            ...
            if flash > 0 {
                ...
                // One solid blink in a unique hue per preset (30° steps).
                let color = preset_color(preset_glob.get() as usize);
                ...
                match latch_layer {
                    LatchLayer::Main => {
                        let chord_type = value_to_index(chord_glob.get(), NUM_CHORD_TYPES);
                        let level =
                            ((chord_type as u32 * 255) / (NUM_CHORD_TYPES as u32 - 1)) as u8;
